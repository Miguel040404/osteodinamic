...
export const UserReservationItem = ({ reserva, onCancel }) => {
  const [stateNoApuntarse, actionNoApuntarse, pendingNoApuntarse] = useActionState(onCancel, {});
  return (
    <div className={`flex flex-col md:flex-row justify-between items-start md:items-center p-5 rounded-xl border ${reserva.horario.tipo === "Entrenamiento" ? "border-[#dbc7b0] bg-[#f4ebe3]" : "border-[#c7b1a1] bg-[#f3e6dc]"}`}>
      ...
      <form action={actionNoApuntarse} className="w-full md:w-auto">
        <input type="hidden" name="horarioId" value={reserva.horario.id} />
        <input type="hidden" name="tipo" value={reserva.horario.tipo} />
        <button disabled={pendingNoApuntarse} type="submit" className="px-4 py-2 text-sm bg-[#fae5e0] text-[#a23b2e] border border-[#e2b4ac] rounded-lg hover:bg-[#f2d4ce] transition cursor-pointer">
          <span className="flex gap-2 items-center">{pendingNoApuntarse ? <LoaderCircle className="w-5 h-5 animate-spin" /> : ''}
            Cancelar reserva
          </span>
        </button>
      </form>
