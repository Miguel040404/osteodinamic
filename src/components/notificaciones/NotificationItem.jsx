...
import { PencilIcon, TrashIcon, BellIcon, ClockIcon } from 'lucide-react';
import NotificationButton from './notification-button';
import { eliminarNotificacion } from '@/lib/actions';


export const NotificacionItem = ({ notificacion, session }) => (
    <div className="bg-[#f5e19106] border border-[#b9b59c] rounded-xl shadow-sm hover:shadow-md transition overflow-hidden">
        <div className="p-6">
            <div className="flex flex-col md:flex-row justify-between gap-4">
                <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h2 className="text-xl font-semibold text-[#4d4037]">{notificacion.title}</h2>
                    </div>
                    <p className="text-[#4d4037] mt-3">{notificacion.message}</p>
                    <div className="flex items-center gap-4 mt-4 text-sm text-[#4d4037]">
                        <div className="flex items-center gap-2">
                            <ClockIcon className="w-4 h-4" />
                            <span>{new Date(notificacion.createdAt).toLocaleString()}</span>
                        </div>
                    </div>
                </div>
